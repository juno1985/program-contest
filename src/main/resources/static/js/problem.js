$(function() {
	...
	$('#href_hist').on('click',function(){
		...
		$.ajax({
			...
			success:function(obj){
				if(obj.code != 0){
					$('#hist').html("<strong>无记录</strong>");
				}else{
					var array_elment = obj.object;
					var array_attr = [ "submitTime", "result" ];
					var array_thead = [ "提交时间", "结果" ];

					// 删除body内所有元素
					$('#hist').children().remove();

					var table = create_table("我的历史", array_thead, array_elment,
							array_attr);

					$('#hist').html(table);
					
					$('#hist').find('.result-btn').on('click',function(){
						var code_id = $(this).val();
						//获取历史代码
						$.ajax({
							url: "/contest/challenge/" + code_id + "/code",
							type: "GET",
							dataType: "json",
							success:function(obj){
								var code_html = obj.object;
								$('#hist').find('#code_div').remove();
								$('#hist').append('<div id="code_div" style="border:2px solid #939393;background-color: #f9f9f9;padding: 10px 10px;">' + code_html + '</div>');
							}
						});
					});
				}
	...
	function create_tbody_4(obj, array_element, array_attr) {
		...
		for (var i = 0; i < array_element.length; i++) {
			...
			for (var j = 0; j < array_attr.length; j++) {
				...
				if(attr == "submitTime"){
					cell_data = element[attr].substring(0,19).replace("T"," ");;
				}
				else if(attr == "result"){
					if(element[attr]=="success"){
						cell_data = "成功";
						cell_data='<button type="button" class="btn btn-success result-btn" value="' + element["id"] +'">' + cell_data + '</button>';
					}
					else {
						cell_data = "失败";
						cell_data='<button type="button" class="btn btn-danger result-btn" value="' + element["id"] +'">' + cell_data + '</button>';
					}
				}
				td += "<td>" + cell_data + "</td>";
